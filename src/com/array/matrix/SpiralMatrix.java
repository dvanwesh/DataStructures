...
private static void fetchSpiralPattern(int[][] matrix, int size,int start, int end, List<Integer> res) {
	if(size==0) return;
	for(int j=start;j<=end;j++) res.add(matrix[start][j]);
	for(int i=start+1;i<=end;i++) res.add(matrix[i][end]);
	for(int j=end-1;j>=start;j--) res.add(matrix[end][j]);
	for(int i=end-1;i>start;i--) res.add(matrix[i][start]);
